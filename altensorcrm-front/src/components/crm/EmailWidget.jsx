...
const EmailWidget = ({
  ...
  const handleSelectTemplate = (template) => {
    ...
    showToast(`Template "${template.name}" tətbiq olundu!`, 'success');
  };
  ...
  const handleSendEmail = async () => {
    ...
    if (!finalToEmail) {
      showToast('Xahiş olunur qəbul edən e-poçt ünvanını qeyd edin.', 'error');
      return;
    }
    ...
    try {
      ...
      showToast('E-poçt uğurla göndərildi və saxlanıldı!', 'success');
      handleDiscard();
      await fetchEmailLogs();
    } catch (err) {
      console.error('Error sending email:', err);
      showToast(err.message || 'E-poçt göndərilərkən xəta baş verdi.', 'error');
    } finally {
      setSending(false);
    }
  };
      ...
      <div className="flex-1 space-y-6">
        ...
        {emailsList.length === 0 && !isComposeOpen && (
          <div className="py-20 flex flex-col items-center justify-center text-center select-none">
            ...
            <h3 className="text-sm font-bold text-white tracking-tight">No Emails Found</h3>
        ...
        {!isComposeOpen ? (
          ...
          <div className="bg-[#141416] border border-[#27272A] rounded-2xl p-4 space-y-3 relative shadow-2xl animate-in fade-in duration-150">
            ...
            <div className="flex items-center gap-3 bg-[#1C1C1E] border border-[#2C2C2E] rounded-xl px-3.5 py-2 text-xs">
              <span className="text-[#71717A] font-bold font-mono uppercase w-10 shrink-0">TO:</span>
              <div className="flex-1 flex items-center gap-2 flex-wrap">
                ...
                <input
                  type="email"
                  placeholder={!toEmailChip ? "Enter recipient email..." : ""}
                  value={toEmailInput}
            ...
            {/* SUBJECT: Row */}
            <div className="flex items-center gap-3 bg-[#1C1C1E] border border-[#2C2C2E] rounded-xl px-3.5 py-2 text-xs">
              <span className="text-[#71717A] font-bold font-mono uppercase w-14 shrink-0">SUBJECT:</span>
              <input
                type="text"
                value={subject}
        ...
        <div className="fixed inset-0 z-[250] bg-black/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-[#1F1F22] border border-[#2C2C2E] rounded-3xl shadow-2xl p-6 w-full max-w-2xl text-[#E4E4E7] space-y-5 animate-in fade-in duration-150 relative">
            ...
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-white tracking-tight">Email Templates</h2>
              <button
                type="button"
            ...
            <div className="min-h-[200px] max-h-[360px] overflow-y-auto custom-scrollbar p-1">
              {filteredTemplates.length === 0 ? (
                <div className="py-16 flex flex-col items-center justify-center text-center space-y-3">
                  <span className="text-sm font-semibold text-[#A1A1AA]">No Templates Found</span>
                  <button
                    type="button"
                    onClick={handleGoToCreateTemplate}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {filteredTemplates.map((template) => (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-[#A1A1AA] font-medium">Subject: {template.subject}</p>

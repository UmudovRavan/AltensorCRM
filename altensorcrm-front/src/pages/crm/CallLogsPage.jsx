    ...
    <div className="space-y-4 max-w-[1600px] mx-auto text-[#D4D4D8] font-sans selection:bg-fuchsia-500/30 relative min-h-[calc(100vh-80px)]">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-semibold text-white relative" ref={viewRef}>
          <span className="text-[#A1A1AA]">Call Logs</span>
      ...
      <div className="flex flex-wrap lg:flex-nowrap items-center justify-between gap-2.5 relative z-20">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="relative" ref={mediumRef}>
            ...
            >
              <span className="truncate">{selectedMediumFilter || 'Telephony Medium'}</span>
          ...
          <div className="relative" ref={typeRef}>
            ...
            >
              <span className="truncate">{selectedTypeFilter || 'Type'}</span>
              <ChevronDownIcon className="w-3.5 h-3.5 text-[#71717A] shrink-0" />
            </button>
              ...
              <div className="absolute top-9 left-0 w-44 bg-[#1C1C1E] border border-[#2C2C2E] rounded-2xl shadow-2xl p-1.5 z-50 flex flex-col text-xs text-[#E4E4E7] animate-in fade-in duration-150">
                ...
                {callTypes.map((t) => (
                  <button
                    ...
                    className={`flex items-center justify-between w-full px-3 py-2 rounded-xl transition-colors text-left cursor-pointer ${
                      selectedTypeFilter === t ? 'bg-[#2C2C2E] text-white font-semibold' : 'hover:bg-[#2C2C2E]/60 text-[#D4D4D8]'
                    }`}
                  >
                    <span>{t}</span>
                    {selectedTypeFilter === t && <CheckIcon className="w-4 h-4 text-sky-400" />}
                  </button>
                ))}
              </div>
            )}
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-[#18181B] border-b border-[#27272A] text-[#71717A] font-medium uppercase tracking-wider text-[11px]">
                  ...
                  {isColVisible('caller') && <th className="py-3 px-4 text-[#A1A1AA] font-normal">Caller</th>}
                  {isColVisible('receiver') && <th className="py-3 px-4 text-[#A1A1AA] font-normal">Receiver</th>}
                  {isColVisible('type') && <th className="py-3 px-4 text-[#A1A1AA] font-normal">Type</th>}
                  {isColVisible('status') && <th className="py-3 px-4 text-[#A1A1AA] font-normal">Status</th>}
                  {isColVisible('duration') && <th className="py-3 px-4 text-[#A1A1AA] font-normal">Duration</th>}
                  {isColVisible('fromNumber') && <th className="py-3 px-4 text-[#A1A1AA] font-normal">From (number)</th>}
                  {isColVisible('toNumber') && <th className="py-3 px-4 text-[#A1A1AA] font-normal">To (number)</th>}
                  {isColVisible('createdOn') && <th className="py-3 px-4 text-[#A1A1AA] font-normal">Created On</th>}
                </tr>
              </thead>
                    ...
                    >
                      ...
                      {isColVisible('type') && (
                        <td className="py-3.5 px-4 text-[#D4D4D8]">
                          <div className="flex items-center gap-1.5">
                            {call.type === 'Incoming' ? (
                              <PhoneArrowDownLeftIcon className="w-4 h-4 text-emerald-400 shrink-0" />
                            ) : (
                              <PhoneArrowUpRightIcon className="w-4 h-4 text-sky-400 shrink-0" />
                            )}
                            <span>{call.type}</span>
                          </div>
                        </td>
                      ...
                      {/* From (number) */}
                      {isColVisible('fromNumber') && (
                        <td className="py-3.5 px-4 text-[#A1A1AA] font-mono">{call.fromNumber}</td>
                      )}

                      {/* To (number) */}
                      {isColVisible('toNumber') && (
                        <td className="py-3.5 px-4 text-[#A1A1AA] font-mono">{call.toNumber}</td>
                      )}

                      {/* Created On */}
                      {isColVisible('createdOn') && (
                        <td className="py-3.5 px-4 text-[#71717A]">{call.createdOn}</td>
                      )}
                    </tr>
                  );
                })}
      ...
      {activeView === 'Kanban' && (
        <div className="flex gap-4 overflow-x-auto custom-scrollbar pb-6 animate-in fade-in duration-200">
          {callStatuses.slice(0, 5).map((st) => {
            const colLogs = filteredCallLogs.filter((c) => c.status === st);
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-white group-hover:text-sky-400 transition-colors">{log.caller}</span>
                        </div>
                        {getStatusBadge(log.status)}
                      </div>

                      <div className="text-xs text-[#A1A1AA]">
                        <p>Receiver: <span className="text-white">{log.receiver}</span></p>
                        <p className="font-mono">From: {log.fromNumber}</p>
                        <p className="font-mono">To: {log.toNumber}</p>
                      </div>

                      <div className="flex items-center justify-between text-[11px] text-[#71717A] pt-2 border-t border-[#2C2C2E]">
        ...
        <div className="bg-[#121214] border border-[#27272A] rounded-2xl overflow-hidden shadow-xl animate-in fade-in duration-200">
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-[#27272A]/60 text-[#D4D4D8]">
                {Object.keys(groupedCallLogs).map((typeGroup) => {
                  const groupItems = groupedCallLogs[typeGroup];
                  const isExpanded = expandedGroups[typeGroup] !== false;

                  return (
                    <React.Fragment key={typeGroup}>
                      <tr className="bg-[#18181B] font-semibold text-white cursor-pointer hover:bg-[#27272A]/60 transition-colors" onClick={() => toggleGroup(typeGroup)}>
                        <td className="py-3 px-4 col-span-full" colSpan={9}>
                          <div className="flex items-center gap-2">
                            <span className="text-[#A1A1AA]">{isExpanded ? '▼' : '▶'}</span>
                            <span className="text-white">Type - {typeGroup}</span>
                          </div>
                        </td>
                      </tr>
                      ...
                      {isExpanded &&
                        groupItems.map((call) => {
                          ...
                          return (
                            <tr key={call.id} className={`hover:bg-[#18181B]/80 transition-colors ${isSelected ? 'bg-[#18181B]' : ''}`}>
                              ...
                              {isColVisible('type') && <td className="py-3.5 px-4 text-[#D4D4D8]">{call.type}</td>}
                              {isColVisible('status') && <td className="py-3.5 px-4">{getStatusBadge(call.status)}</td>}
                              {isColVisible('duration') && <td className="py-3.5 px-4 text-[#A1A1AA]">{call.duration}</td>}
                              {isColVisible('fromNumber') && <td className="py-3.5 px-4 font-mono text-[#A1A1AA]">{call.fromNumber}</td>}
                              {isColVisible('toNumber') && <td className="py-3.5 px-4 font-mono text-[#A1A1AA]">{call.toNumber}</td>}

...
const OrganizationDetailPage = () => {
  ...
  const orgTitle = formData.organizationName || 'Organization Details';
    ...
    <div className="-m-4 lg:-m-6 -mb-20 min-h-screen bg-[#121214] text-[#E4E4E7] flex flex-col font-sans relative">
      ...
      <div className="px-6 py-3 border-b border-[#2C2C2E]/60 bg-[#121214] flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2 text-xs font-medium text-[#A1A1AA]">
          <Link to="/crm/organizations" className="hover:text-white transition-colors">Organizations</Link>
          <span>/</span>
          <Link to="/crm/organizations" className="hover:text-white transition-colors">List</Link>
          <span>/</span>
          <span className="text-white font-semibold">{orgTitle}</span>
        </div>
      </div>

      {/* 2. MAIN TWO-COLUMN CONTENT BODY (Screenshot 1 & 2 Match!) */}
      <div className="flex-1 flex flex-col lg:flex-row min-w-0">
        ...
        <div className="w-full lg:w-80 shrink-0 border-r border-[#2C2C2E]/60 bg-[#121214] p-6 space-y-6 text-xs overflow-y-auto custom-scrollbar">
          ...
          {/* Details Section (Screenshot 1 & 2 Match!) */}
            ...
            {isDetailsOpen && (
              <div className="space-y-3.5 text-xs">
                ...
                <div className="space-y-1">
                  <label className="text-[#71717A] text-[11px] block">Organization Name</label>
                  <input
                    type="text"
                ...
                {/* Website */}
                <div className="space-y-1">
                  <label className="text-[#71717A] text-[11px] block">Website</label>
                  <input
                    type="text"
                ...
                {/* Territory */}
                <div className="space-y-1">
                  <label className="text-[#71717A] text-[11px] block">Territory</label>
                  <select
                  ...
                  >
                    <option value="" className="bg-[#1C1C1E]">Add Territory...</option>
                    {territoryOptions.map(t => <option key={t} value={t} className="bg-[#1C1C1E]">{t}</option>)}
                  </select>
                </div>

                {/* Industry */}
                <div className="space-y-1">
                  <label className="text-[#71717A] text-[11px] block">Industry</label>
                  <select
                    ...
                    className="w-full bg-transparent border-none text-white focus:outline-none focus:ring-1 focus:ring-sky-500 rounded px-1 py-1 cursor-pointer"
                  >
                    {industryOptions.map(i => <option key={i} value={i} className="bg-[#1C1C1E]">{i}</option>)}
                  </select>
                </div>

                {/* No. of Employees */}
                <div className="space-y-1">
                  <label className="text-[#71717A] text-[11px] block">No. of Employees</label>
                  <select
                ...
                {/* Address */}
                <div className="space-y-1">
                  <label className="text-[#71717A] text-[11px] block">Address</label>
                  <input
                    type="text"
        ...
        {/* RIGHT MAIN PANEL: DEALS & CONTACTS TABS (Screenshot 1 & 2 Match!) */}
        <div className="flex-1 p-6 lg:p-8 flex flex-col min-w-0">
          ...
          {/* TAB 1: DEALS TAB (Screenshot 1 Match!) */}
          {activeTab === 'Deals' && (
            <div className="mt-4 flex-1 flex flex-col">
              {associatedDeals.length === 0 ? (
                /* Empty State (Screenshot 1 Match!) */
                <div className="py-24 flex flex-col items-center justify-center text-center">
                  ...
                  <h3 className="text-sm font-bold text-white mt-4">No Deals Found</h3>
                  <p className="text-xs text-[#A1A1AA] max-w-sm mt-1 leading-relaxed">
                    It appears that there are currently no Deals available. You can create more Deals by using the Create button.
                  </p>
                </div>
              ) : (
          ...
          {/* TAB 2: CONTACTS TAB (Screenshot 2 Match!) */}
          {activeTab === 'Contacts' && (
            <div className="mt-4 flex-1 flex flex-col">
              {associatedContacts.length === 0 ? (
                ...
                <div className="py-24 flex flex-col items-center justify-center text-center">
                  ...
                  <h3 className="text-sm font-bold text-white mt-4">No Contacts Found</h3>
                  <p className="text-xs text-[#A1A1AA] max-w-sm mt-1 leading-relaxed">
                    No contacts found for this organization.
                  </p>
                </div>
              ) : (
                /* Real Database Contacts Table (Screenshot 2 Match!) */
                <div className="overflow-x-auto custom-scrollbar">
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="border-b border-[#2C2C2E]/60 text-[#A1A1AA] font-medium">
